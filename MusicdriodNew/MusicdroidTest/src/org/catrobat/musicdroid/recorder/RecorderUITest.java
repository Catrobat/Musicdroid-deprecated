...
public class RecorderUITest extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	public void testOrientation() {
		assertEquals("Not in Landscape mode!", Configuration.ORIENTATION_LANDSCAPE, solo.getCurrentActivity()
				.getResources().getConfiguration().orientation);
	}

	public void testPlayerBoxGoneAtStart() {
		RelativeLayout progressBarBox = (RelativeLayout) solo.getCurrentActivity().findViewById(
				R.id.microphone_progress_bar_box);
	...
	public void testImportButtonInvisibleAtStart() {
		RelativeLayout addBox = (RelativeLayout) solo.getCurrentActivity().findViewById(
				R.id.microphone_add_to_sound_mixer_box);
	...
	public void testDoNotRecordAtDialogAbort() {
		...
		File f = new File(((RecorderActivity) solo.getCurrentActivity()).getCurrentRecordingSession().getPathToFile());
		long modified = 0;
		if (f.exists())
		...
		f = new File(((RecorderActivity) solo.getCurrentActivity()).getCurrentRecordingSession().getPathToFile());
		if (f.exists()) {
	...
	public void testChangeFilename() {
		...
		solo.sleep(1000);
		String filename = ((RecorderActivity) solo.getCurrentActivity()).getCurrentRecordingSession().getFilename();
		...
		File f = new File(((RecorderActivity) solo.getCurrentActivity()).getCurrentRecordingSession().getPath() + "/"
				+ testFilename);
		assertTrue(f.exists());
	}
