...
import org.catrobat.musicdroid.soundtrack.SoundTrackDummy;
...
public class UITestHelper {
	...
	public void addDummyTrack(final ActivityInstrumentationTestCase2<MainActivity> test) {
		new Thread() {
	        @Override
	        public void run() {
               try {
            	   test.runTestOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                    		SoundTrackDummy stvd = new SoundTrackDummy();
                    		((MainActivity)test.getActivity()).addSoundTrack(
                    				new SoundTrackView(test.getActivity(), stvd));
                        }
                     });
               } catch (Throwable e) {
				e.printStackTrace();
